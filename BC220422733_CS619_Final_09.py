...
# Data augmentation functions (apply to all displayed images)
def rotation():
    global processed_images_cv
    for i in range(len(processed_images_cv)):
        ...
        processed_images_cv[i] = cv2.warpAffine(processed_images_cv[i], M, (w, h))
    update_image_display()
...
def main_gui(root):
    ...
    # Add buttons for image processing and augmentation functions
    ...
    # Add remaining buttons for other augmentation functions...
    # ... (Repeat similar for noise reduction, skull stripping, etc.)
