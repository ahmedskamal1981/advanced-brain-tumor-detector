...
def reset():
    global uploaded_image_paths, processed_images_cv, test_image_paths, test_labels, test_predictions, single_uploaded_image_path
    uploaded_image_paths = []
    processed_images_cv = []
    test_image_paths = []
    test_labels = []
    test_predictions = []
    single_uploaded_image_path = None
    for label in image_labels:
        label.config(image='')  # Clear image display
    single_image_label.config(image='')  # Clear single image display
    single_result_label.config(text="")  # Clear single image result
...
def generate_confusion_matrix():
    ...
    # Confusion Matrix creation using Matplotlib
...
# New Functionality: Upload Single Image
def upload_single_image():
    global single_uploaded_image_path
    single_uploaded_image_path = filedialog.askopenfilename(
        title="Select an Image",
        filetypes=[("Image Files", "*.jpg *.jpeg *.png")]
    )
    if single_uploaded_image_path:
        # Display the single image
        img = Image.open(single_uploaded_image_path)
        img = img.resize((300, 300), Image.LANCZOS)
        img_tk = ImageTk.PhotoImage(img)
        single_image_label.config(image=img_tk)
        single_image_label.image = img_tk  # Keep a reference to avoid garbage collection

        # Predict and display the result
        result = predict_image(single_uploaded_image_path)
        single_result_label.config(text=f"{'Tumor Detected' if result == 1 else 'No Tumor Detected'}", fg='green' if result == 0 else 'red')
...
def main_gui(root):
    ...
    # Upload Single Image Button (New)
    single_upload_button = Button(button_frame, text="Upload Single Image", command=upload_single_image, font=("times new roman", 12), bg="magenta", fg="white")
    single_upload_button.pack(pady=20)
    ...
    # Create a frame for single image display and result
    single_image_frame = Frame(frame, bg="white", bd=2, relief="groove")
    single_image_frame.grid(row=1, column=2, padx=10, pady=10, sticky="n")

    single_title = Label(single_image_frame, text="Single Image Prediction", font=("times new roman", 16, "bold"), bg="white")
    single_title.pack(pady=10)

    global single_image_label
    single_image_label = Label(single_image_frame, bg="lightgrey", width=300, height=300)
    single_image_label.pack(padx=10, pady=10)

    global single_result_label
    single_result_label = Label(single_image_frame, text="", font=("times new roman", 16, "bold"), bg="white")
    single_result_label.pack(pady=10)
