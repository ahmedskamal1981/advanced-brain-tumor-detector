...
def train_model():
    dataset_path = folder_path.get()
    if not dataset_path:
        messagebox.showerror("Error", "Please upload a dataset folder.")
        return
    ...
    evaluate_model(train_generator, validation_generator)
...
def evaluate_model(train_generator, validation_generator):
    ...
    report = classification_report(validation_labels, predicted_classes, target_names=["No Tumor", "Tumor"])
    print(report)
...
def upload_dataset():
    ...
    if path:
        ...
        messagebox.showinfo("Dataset Loaded", "Dataset folder has been loaded successfully.")
...
def main_gui(root):
    ...
    upload_button = Button(upload_frame, text="Upload Dataset Folder", command=upload_dataset, font=("times new roman", 12), bg="orange", fg="white")
