...
def train_model(image_folder):
    ...
    train_generator = train_datagen.flow_from_directory(
        image_folder,  # Use the uploaded dataset path
        target_size=(128, 128),
        batch_size=32,
        class_mode='binary',
        subset='training')
...
def display_images(folder):
    global image_label, image_list, img_index
    image_list = glob.glob(os.path.join(folder, "*.jpg")) + \
                 glob.glob(os.path.join(folder, "*.jpeg")) + \
                 glob.glob(os.path.join(folder, "*.png"))
    img_index = 0  # Start from the first image
    if image_list:
        show_image(image_list[img_index])
    else:
        messagebox.showerror("Error", "No images found in the selected folder.")

def show_image(image_path):
    global img, img_tk, img_cv
...
def main_gui(root):
    ...
    upload_button = Button(upload_frame, text="Upload Dataset", command=upload_dataset, font=("times new roman", 12), bg="orange", fg="white")
